...
// Redux action ni dispatch ga bog'laymiz
const actionDispatch = (dispatch: Dispatch) => ({
  setProducts: (data: Product[]) => dispatch(setProducts(data)),
});

// Redux store dan products ni olamiz
...
export default function Products(props: ProductsProps) {
  ...
  // Qidiruv filtri — sahifa, limit, tartib, kategoriya, qidiruv matni
  ...
  const [searchText, setSearchText] = useState<string>(""); // input qiymati
  const history = useHistory(); // sahifalar orasida o'tish uchun

  // productSearch o'zgarganda serverdan mahsulotlarni qayta oladi
  useEffect(() => {
  ...
  // searchText bo'shatilsa — qidiruvni ham tozalaydi
  useEffect(() => {
    if (searchText === "") {
  ...
  // Kategoriya tugmasi bosilganda — sahifani 1 ga qaytarib, kategoriyani o'zgartiradi
  ...
  // Tartib tugmasi bosilganda — sahifani 1 ga qaytarib, tartibni o'zgartiradi
  ...
  // SEARCH tugmasi bosilganda — searchText ni filtrga yuboradi
  const searchProductHandler = () => {
    productSearch.search = searchText;
    setProductSearch({ ...productSearch });
  };

  // Pagination tugmasi bosilganda — sahifa raqamini yangilaydi
  const paginationHandler = (e: ChangeEvent<any>, value: number) => {
    productSearch.page = value;
    setProductSearch({ ...productSearch });
  };

  // Mahsulot kartasi bosilganda — mahsulot sahifasiga o'tadi
    ...
    <div className={"products"}>
      <Container>
        <Stack flexDirection={"column"} alignItems={"center"}>
          ...
          <Stack className={"avatar-big-box"}>
            ...
            <Box className={"single-search-big-box"}>
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") searchProductHandler(); // Enter bosilsa qidiradi
                }}
              />
              <button
          ...
          {/* ── TARTIB FILTRI — New / Price / Views ── */}
          <Stack className={"dishes-filter-section"}>
            <Stack className={"dishes-filter-box"}>
              <Button
                variant={"contained"}
                color={productSearch.order === "createdAt" ? "primary" : "secondary"} // tanlangan bo'lsa primary rang
                className={"order"}
            ...
            <Stack className={"product-wrapper"}>
              {products.length !== 0 ? (
                products.map((product: Product) => {
                  const imagePath = `${serverApi}/${product.productImages[0]}`; // rasm yo'li
                  const sizeVolume =
                    product.productCollection === ProductCollection.DRINK
                      ? product.productVolume + "litre" // ichimlik — litre
                      : product.productSize + "size";   // taom — size

                  return (
                    <Stack
                      key={product._id}
                      className={"product-card"}
                      onClick={() => chooseDishHandler(product._id)} // kartaga bosilsa mahsulot sahifasiga o'tadi
                    >
                      <Stack
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            e.stopPropagation(); // karta onClick i ishlamasin
                          }}
                        >
                ...
                <Box className="no-data">Products are not available!</Box> // mahsulot yo'q
              )}
            </Stack>
          </Stack>
          ...
          <Stack className={"pagination-section"}>
            <Pagination
              count={
                products.length !== 0
                  ? productSearch.page + 1 // mahsulot bor — keyingi sahifa ham ko'rsatiladi
                  : productSearch.page     // mahsulot yo'q — faqat joriy sahifa
              }
      ...
      <div className={"brands-logo"}>
        <Container className={"family-brands"}>
          ...
          <Stack className={"brand-list"}>
            ...
            <Box className={"review-box"}><img src={"/img/seafood.webp"} /></Box>
            <Box className={"review-box"}><img src={"/img/sweets.webp"} /></Box>
            <Box className={"review-box"}><img src={"/img/doner.webp"} /></Box>
          </Stack>
        </Container>
      </div>
